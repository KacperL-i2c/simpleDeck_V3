...
static enum usbd_request_return_codes hid_control_request(usbd_device *dev,
    ...
    /* USB_DT_HID (0x21) zwracamy automatycznie z pola `extra` deskryptora
     * interfejsu - libopencm3 to obsługuje. */
    return USBD_REQ_NEXT_CALLBACK;
}
...
void usbhid_init(void) {
     ...
     */
    SET_REG(USB_CNTR_REG, USB_CNTR_PWDN);
    ...
    USB_SET_EP_ADDR(0, 0);
    USB_SET_EP_TYPE(0, USB_EP_TYPE_CONTROL);
    USB_SET_EP_TX_ADDR(0, 0x40);
    USB_CLR_EP_TX_DTOG(0);
    USB_SET_EP_TX_STAT(0, USB_EP_TX_STAT_NAK);
    USB_SET_EP_RX_ADDR(0, 0x80);
    USB_SET_EP_RX_COUNT(0, 0x8400);  /* BL_SIZE=1, NUM_BLOCK=1 → 64 B */
    USB_CLR_EP_RX_DTOG(0);
    USB_SET_EP_RX_STAT(0, USB_EP_RX_STAT_VALID);
